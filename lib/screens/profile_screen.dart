...
class _ProfileScreenState extends State<ProfileScreen> {
              ...
              actions: [
                ...
                ElevatedButton(
                  ...
                  onPressed: isSaving ? null : () async {
                    ...
                    final response = await ApiService.updateProfile(widget.userId, newName);
                    ...
                    if (response['message'] == 'Profil berhasil diperbarui') {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(response['message'] ?? 'Gagal memperbarui')));
                    }
                  },
                  child: isSaving
      ...
      builder: (context) => StatefulBuilder(
          builder: (context, setStateDialog) {
            return AlertDialog(
              backgroundColor: AppColors.cardBackground,
              shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
              title: const Text('Ubah Password', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
              content: Column(
                ...
                children: [
                  TextField(
                    ...
                    obscureText: true,
                    decoration: InputDecoration(labelText: 'Password Lama', filled: true, fillColor: AppColors.background, border: OutlineInputBorder(borderRadius: BorderRadius.circular(10), borderSide: BorderSide.none)),
                  ),
                  const SizedBox(height: 12),
                  TextField(
                    controller: newPassController,
                    obscureText: true,
                    decoration: InputDecoration(labelText: 'Password Baru', filled: true, fillColor: AppColors.background, border: OutlineInputBorder(borderRadius: BorderRadius.circular(10), borderSide: BorderSide.none)),
                  ),
                ],
              ),
              actions: [
                ...
                ElevatedButton(
                  ...
                  onPressed: isSaving ? null : () async {
                    ...
                    setStateDialog(() => isSaving = true);
                    final response = await ApiService.updatePassword(widget.userId, oldPassController.text, newPassController.text);
                    if (!context.mounted) return;
                    ...
                    if (response['message'] == 'Password berhasil diubah') {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(response['message'] ?? 'Gagal mengubah password')));
                    }
                  },
                  child: isSaving
                      ? const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2))
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          leading: IconButton(
            ...
            onPressed: () => Navigator.pop(context, _currentUsername),
          ),
        ),
        body: SingleChildScrollView(
          ...
          child: Column(
            children: [
              ...
              _buildMenuTile(icon: Icons.lock_outline, title: 'Ubah Password', onTap: _showChangePasswordDialog),

              Container(
                margin: const EdgeInsets.only(bottom: 12),
                decoration: BoxDecoration(color: AppColors.cardBackground, borderRadius: BorderRadius.circular(12)),
                child: ListTile(
                  leading: Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: AppColors.background, borderRadius: BorderRadius.circular(8)), child: const Icon(Icons.notifications_outlined, color: AppColors.primary, size: 20)),
                  title: const Text('Pengaturan Notifikasi', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: AppColors.textPrimary)),
                  trailing: Switch(
  ...
  Widget _buildMenuTile({required IconData icon, required String title, required VoidCallback onTap}) {
    return Container(
      ...
      decoration: BoxDecoration(color: AppColors.cardBackground, borderRadius: BorderRadius.circular(12)),
      child: ListTile(
        leading: Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: AppColors.background, borderRadius: BorderRadius.circular(8)), child: Icon(icon, color: AppColors.primary, size: 20)),
